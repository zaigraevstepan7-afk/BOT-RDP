...
namespace color_picker {
    ...
    inline ImVec4 picker_circle_shadow = ImColor(15, 5, 5);
...
namespace button {
    ...
    inline ImVec4 button_bg = ImColor(24, 24, 24);
    inline ImVec4 button_gradient = ImColor(22, 23, 25);
...
namespace c
{
    ...
    namespace background
    {
        ...
        inline ImVec4 stroke = ImColor(125,125,125);
        inline ImVec2 size = ImVec2(1000 , 660);

        inline float rounding = 15;
    ...
    namespace tab
    {
        inline ImVec4 tab_active = ImColor(15,15,15);

        inline ImVec4 border = ImColor(15,15,15);
    }
...
namespace menuConfig {
	inline ImVec4 accent = ImColor(110, 110, 210);
};
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(w, 60 * 1.1f));
    ...
    if (it_anim == anim.end())
    {
	 ...
	 const float shadow_alpha = 0.03f;
 const int shadow_steps = 12;
 const float max_spread = 8.0f;

 for (int i = shadow_steps; i > 0; i--)
 {
     float ratio = (float)i / (float)shadow_steps;
     GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(max_spread * ratio, max_spread * ratio),bb.Max + ImVec2(max_spread * ratio, max_spread * ratio),GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, shadow_alpha * ratio * 0.5f)),c::elements::rounding );
 }
    ...
    it_anim->second.outline = ImLerp(it_anim->second.outline, it_anim->second.active ? menuConfig::accent : button::button_bg, g.IO.DeltaTime * 6.f);

    // ИЗМЕНЕННЫЙ ЦВЕТ ФОНА: 18,18,18
    const ImColor bg_color(18, 18, 18);
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max, bg_color, 7.f);

    // Эффект при нажатии остается
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max, GetColorU32(menuConfig::accent, 0.1f * it_anim->second.alpha), 5.f);

    // Обводка остается без изменений
    ImGui::GetWindowDrawList()->AddRect(bb.Min + ImVec2(1, 1), bb.Max, GetColorU32(it_anim->second.outline), 5.f);

    // Текст остается без изменений
    ImGui::GetWindowDrawList()->AddText(
        ImVec2(
            bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, 
            bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2
        ), 
        GetColorU32(button::text_active), 
        label
    );
...
void AddUniformShadow(ImDrawList* draw_list, const ImVec2& min, const ImVec2& max, 
                      ImU32 shadow_color, float shadow_size, float rounding)
{
    const int steps = 12; // Больше шагов для более плавной тени
    const int base_alpha = (shadow_color >> 24) & 0xFF;
    
    // Расширяем область для тени
    const float total_expansion = shadow_size * 1.5f;
    const ImVec2 expanded_min = min - ImVec2(total_expansion, total_expansion);
    const ImVec2 expanded_max = max + ImVec2(total_expansion, total_expansion);
    
    // Рисуем основную тень
    draw_list->AddRectFilled(expanded_min, expanded_max, shadow_color, rounding + total_expansion);
    
    // Рисуем серию размывающих колец
    for (int i = 1; i <= steps; i++)
    {
        const float t = (float)i / steps;
        const float alpha_factor = 1.0f - t * t * t; // Кубическое затухание для гладкости
        const int current_alpha = static_cast<int>(base_alpha * alpha_factor);
        const float expansion = t * shadow_size;
        
        const ImVec2 ring_min = min - ImVec2(expansion, expansion);
        const ImVec2 ring_max = max + ImVec2(expansion, expansion);
        
        draw_list->AddRect(
            ring_min,
            ring_max,
            IM_COL32(0, 0, 0, current_alpha),
            rounding + expansion,
...
// Обновленная функция Tab
bool ImGui::Tab(bool selected, const char* icon, const char* label, const char* description, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    // Инициализация shadow_alpha если нужно
    if (anim.find(id) == anim.end()) {
        state.shadow_alpha = 0.0f;
    }

    // Анимация состояний
    state.icon = ImLerp(state.icon, selected ? menuConfig::accent : hovered ? c::elements::text_hov : c::elements::text, g.IO.DeltaTime * 6.f);
    ...
    // Убрана анимация background_fill (не используется)
    state.shadow_alpha = ImLerp(state.shadow_alpha, selected ? 1.0f : 0.0f, g.IO.DeltaTime * 8.f);

    // Рисуем тень
    if (state.shadow_alpha > 0.01f) {
        const float shadow_size = 8.5f;
        const float base_alpha = 0.06f;
        
        const ImU32 shadow_color = GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, base_alpha * state.shadow_alpha));
    ...
    // НОВЫЙ ЦВЕТ ФОНА: RGB(18,18,18) с прозрачностью из state.bg_alpha
    const ImColor bg_color(18, 18, 18, static_cast<int>(state.bg_alpha * 255));
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, bg_color, c::elements::rounding);

    // Рендерим текст и иконку
    RenderTextColor(font::lexend_bold, bb.Min + ImVec2(size.y, 0), bb.Max, GetColorU32(state.text), label, ImVec2(0.1, 0.5));
    RenderTextColor(font::icomoon, bb.Min, bb.Min + ImVec2(size.y, size.y), GetColorU32(state.icon), icon, ImVec2(0.5, 0.58));
...
struct check_state
{
    ImVec4 background, backgroun, text;
    float alpha, mark_pos;
	float shadow_alpha, t_anim;
};
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = 20.f * 1.2f;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + ImVec2(w, 60 * 1.2f));
 ...
 {
     float ratio = (float)i / (float)shadow_steps;
     GetWindowDrawList()->AddRectFilled(rect.Min - ImVec2(max_spread * ratio, max_spread * ratio),rect.Max + ImVec2(max_spread * ratio, max_spread * ratio),GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, shadow_alpha * ratio * 0.5f)),c::elements::rounding );
 }
    
    state.background = ImLerp(state.background, *v ? menuConfig::accent : c::elements::background_widget, g.IO.DeltaTime * 6.f);
    state.alpha = ImLerp(state.alpha, *v ? 1.f : 0.f, g.IO.DeltaTime * 6.f);
    state.shadow_alpha = ImLerp(state.shadow_alpha, *v ? 1.0f : 0.0f, g.IO.DeltaTime * 8.f);
    state.mark_pos = ImClamp(state.mark_pos + (35.f * g.IO.DeltaTime * (*v ? -3.f : 3.f)), 0.f, 35.f);
	
    const ImColor bg_color(18, 18, 18); // Альфа=255 (непрозрачный)

    // 2. Основной фон
    GetWindowDrawList()->AddRectFilled(
        rect.Min, 
        rect.Max, 
        bg_color, // Используем ImColor напрямую
        c::elements::rounding
    );

    // 3. Правый прямоугольник
    GetWindowDrawList()->AddRectFilled(
        rect.Min + ImVec2(w - 40, 0), 
        rect.Max, 
        bg_color, 
        c::elements::rounding
    );

    // 4. Градиент (с прозрачностью по краям)
    GetWindowDrawList()->AddRectFilledMultiColor(
        rect.Min, 
        rect.Max - ImVec2(20, 0),
        ImColor(18, 18, 18, 0),    // Левый-верхний (прозрачный)
        bg_color,                  // Правый-верхний (непрозрачный)
        bg_color,                  // Правый-нижний (непрозрачный)
        ImColor(18, 18, 18, 0)     // Левый-нижний (прозрачный)
    );

    ImU32 border_color;
    if (*v) {
        border_color = GetColorU32(state.background); // Цвет для активного состояния
    } else {
        // Светло-серый цвет для неактивного состояния (RGB: 200,200,200, альфа: 0.7)
        border_color = GetColorU32(ImVec4(0.40f, 0.40f, 0.40f, 0.4f));
    }
    
    GetWindowDrawList()->AddRect(
        rect.Max - ImVec2(47 * 1.2f, 47 * 1.2f),
        rect.Max - ImVec2(13, 13),
        border_color, // Используем новый цвет границы
        2.f
    );
    ...
    GetWindowDrawList()->AddRectFilled(
        rect.Max - ImVec2(47 * 1.2f, 47 * 1.2f),
        rect.Max - ImVec2(13, 13),
        GetColorU32(state.background, 0.5f),
        2.f
    );
	
    PushClipRect(rect.Max - ImVec2(47 * 1.2f, 47 * 1.2f), rect.Max - ImVec2(13 , 13), true);
    RenderCheckMark(GetWindowDrawList(), rect.Max - ImVec2(40 * 1.2f - (square_sz / 2) / 2 * 1.2f, 40 * 1.2f - (square_sz / 2 + state.mark_pos) / 2 * 1.2f) , GetColorU32(c::elements::mark, state.alpha), square_sz / 2);
    PopClipRect();

    RenderTextColor(font::lexend_bold, rect.Min + ImVec2(10, 0), rect.Max, GetColorU32(c::elements::text_active), label, ImVec2(0.0, 0.5 ));
...
bool ImGui::Switch(const char* label, bool* v) {
    ...
    const float w = GetContentRegionMax().x - style.WindowPadding.x;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + ImVec2(w, 50 * 1.2f));
    ...
    // Инициализация состояния при первом использовании
    if (anim.find(id) == anim.end()) {
        state.t_anim = *v ? 1.0f : 0.0f;
        state.background = *v ? menuConfig::accent : c::elements::background_widget;
    }
    ...
    for (int i = shadow_steps; i > 0; i--)
    {
        float ratio = (float)i / (float)shadow_steps;
        GetWindowDrawList()->AddRectFilled(
            rect.Min - ImVec2(max_spread * ratio, max_spread * ratio),
            rect.Max + ImVec2(max_spread * ratio, max_spread * ratio),
            GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, shadow_alpha * ratio * 0.5f)),
            c::elements::rounding
        );
    }
    ...
    // Основной фон
    const ImVec4 target_bg = *v ? menuConfig::accent : ImVec4(0.12f, 0.12f, 0.12f, 1.0f);
    state.background = ImLerp(state.background, target_bg, g.IO.DeltaTime * 12.f);
    
    // Основной фон виджета (не изменяется)
    GetWindowDrawList()->AddRectFilled(
        rect.Min, 
        rect.Max, 
        GetColorU32(ImVec4(0.07f, 0.07f, 0.07f, 1.0f)),
        c::elements::rounding
    );

    // ============ ОПТИМАЛЬНЫЙ SWITCH ============ //
    const float switch_height = rect.GetHeight() * 0.5f;  // Золотая середина
    const float switch_width = switch_height * 1.8f;      // Гармоничное соотношение
    const float radius = switch_height * 0.45f;          // Чуть меньше половины
    
    // Позиционирование
    const ImVec2 switch_pos_min(
        rect.Max.x - switch_width - 12.0f,               // Компактный отступ
        rect.GetCenter().y - switch_height/2
    );
    const ImVec2 switch_pos_max(
        switch_pos_min.x + switch_width,
        switch_pos_min.y + switch_height
    );
    
    // Плавная анимация
    state.t_anim = ImLerp(state.t_anim, *v ? 1.0f : 0.0f, g.IO.DeltaTime * 14.0f);
    
    // Цвета
    const ImU32 bg_off = GetColorU32(ImVec4(0.12f, 0.12f, 0.12f, 1.0f));
    const ImU32 bg_on = GetColorU32(state.background);
    ImU32 bg_color;
    if (*v) {
        // Включенное состояние - акцентный цвет с анимацией
        bg_color = GetColorU32(ImVec4(
        menuConfig::accent.x / 2,  // Уменьшаем R на 40%
        menuConfig::accent.y / 2,  // Уменьшаем G на 40%
        menuConfig::accent.z / 2,  // Уменьшаем B на 40%
        0.3f              // Альфа остаётся непрозрачной
    ));
    
    
    } else {
        // Выключенное состояние - темный серый с прозрачностью
        bg_color = GetColorU32(ImVec4(0.15f, 0.15f, 0.15f, 0.8f));
    }
    ...
    GetWindowDrawList()->AddRect(
        switch_pos_min, 
        switch_pos_max, 
        GetColorU32(ImVec4(0.3f, 0.3f, 0.3f, 0.4f)), 
        radius, 
        0, 
        1.2f
    );
    ...
    const float circle_radius = radius * 0.9f;
    const float circle_x = ImLerp(
        switch_pos_min.x + circle_radius * 1.1f, 
        switch_pos_max.x - circle_radius * 1.1f, 
        state.t_anim
    );
    const ImVec2 circle_center(circle_x, switch_pos_min.y + switch_height/2);

    // Тень кружка
    // ... предыдущий код ...

// Тень кружка (без изменений)
GetWindowDrawList()->AddCircleFilled(
    circle_center + ImVec2(0, 1), 
    circle_radius + 5, 
    GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.2f))
);

// Сам кружок - НОВЫЙ ЦВЕТ (светлый акцент)
GetWindowDrawList()->AddCircleFilled(
    circle_center,
    circle_radius + 5,
    GetColorU32(menuConfig::accent)
);

// Эффект свечения (без изменений)
if (*v) {
    GetWindowDrawList()->AddCircle(
        circle_center,
        circle_radius * 1.1f,
        GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 0.15f)),
        2.0f
    );
}
...
// ... оставшийся код ...
    ...
    // Текст метки
    RenderTextColor(font::lexend_bold, 
        rect.Min + ImVec2(12, 0), 
        rect.Max, 
        GetColorU32(c::elements::text_active), 
        label, 
        ImVec2(0.0, 0.5)
    );
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10 * 1.1f, 50 * 1.1f), window->DC.CursorPos + ImVec2(w - 10 * 1.1f, 67 * 1.1f));
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 80 * 1.1f));
    ...
    const ImColor bg_color(18, 18, 18);
    GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, bg_color, c::elements::rounding);
    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::elements::background_widget), 5.0f);

    ImRect grab_bb;
    const bool value_changed = SliderBehavior(ImRect(frame_bb.Min - ImVec2(0, 0), frame_bb.Max + ImVec2(10, 0)), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);

    if (value_changed) MarkItemEdited(id);

    // Плавная анимация с экспоненциальным сглаживанием
    const float target_pos = grab_bb.Min.x - (frame_bb.Min.x + 5);
    const float animation_speed = 3.5f; // Регулирует скорость анимации
    state.slow += (target_pos - state.slow) * ImMin(g.IO.DeltaTime * animation_speed, 1.0f);

    // Отрисовка анимированного заполнения
...
// Отрисовка анимированного заполнения
    ...
    frame_bb.Min, 
    frame_bb.Max, 
    ImColor(16, 16, 16), // Темный цвет фона
    7.5f
);
...
GetWindowDrawList()->AddRectFilled(
    frame_bb.Min, 
    ImVec2(frame_bb.Min.x + state.slow + 9, frame_bb.Max.y),
    GetColorU32(ImVec4(
        menuConfig::accent.x / 2,  // Уменьшаем R на 40%
        menuConfig::accent.y / 2,  // Уменьшаем G на 40%
        menuConfig::accent.z / 2,  // Уменьшаем B на 40%
...
// Добавляем кружок на конце полоски
{
    // Рассчитываем позицию кружка (центр правого края заполненной области)
    const float circle_x = frame_bb.Min.x + state.slow + 9;
    const float circle_y = (frame_bb.Min.y + frame_bb.Max.y) * 0.5f; // по вертикали
    
    // Параметры кружка
    const float circle_radius = 10.0f; // размер кружка
    const ImU32 circle_color = IM_COL32(255, 255, 255, 255); // белый цвет
    
	ImVec4 light_accent = ImLerp(menuConfig::accent, ImVec4(1.0f, 1.0f, 1.0f, 1.0f), 0.7f);
    // Рисуем кружок
    GetWindowDrawList()->AddCircleFilled(
...
// ... остальной код (текст) без изменений ...

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    // Одинаковое смещение по Y (0 пикселей) + выравнивание по верхнему краю (0.0)
RenderTextColor(
    font::lexend_bold, 
    rect.Min + ImVec2(10, 0),  // Смещение по Y: 0
    rect.Max, 
    GetColorU32(c::elements::text_active), 
    label, 
    ImVec2(0.0, 0.0)  // Вертикальное выравнивание: верх (0.0)
);

RenderTextColor(
    font::lexend_bold, 
    rect.Min + ImVec2(0, 0),   // Смещение по Y: 0 (как у label)
    rect.Max - ImVec2(15, 10), 
    GetColorU32(c::elements::text_active), 
    value_buf, 
    ImVec2(1.0, 0.0)  // Вертикальное выравнивание: верх (0.0)
);
    return value_changed;
}
...
{
    return ColorEdit4(label, col, ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoAlpha);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const float square_sz = 18.f * 1.1f;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ItemHoverable(g.LastItemData.Rect, g.LastItemData.ID, ImGuiActivateFlags_None) && g.IO.MouseClicked[0] || state.active && !search_col && g.IO.MouseClicked[0] && !state.hovered)
        ...
        // ИЗМЕНЕНИЕ: Фон окна выбора цвета на RGB(18,18,18)
        GetColorU32(color_picker::picker_bg);
        PushStyleColor(ImGuiCol_Border, GetColorU32(color_picker::picker_rect));

        PushStyleVar(ImGuiStyleVar_Alpha, state.alpha);
        PushStyleVar(ImGuiStyleVar_WindowRounding, 1);
        ...
        if (state.active)
        {
            ...
            Begin("picker", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize);
            {
                ...
                // Удаляем эту строку:
// GetWindowDrawList()->AddText(font::lexend_bold, 16.f, GetCursorScreenPos() - ImVec2(0, 2), GetColorU32(color_picker::text_active), (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) ? label : "Color picker");

// И изменяем следующую строку, убирая отступ в 30 пикселей:
ImGui::SetCursorPosY(GetCursorPosY()); // Было +30
                picker_active_window = g.CurrentWindow;
                ImGuiColorEditFlags picker_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaBar;
                ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaPreviewHalf;

                SetNextItemWidth(square_sz * 15.5f);
    ...
    state.text = ImLerp(state.text, state.active ?color_picker::text_active  : color_picker::text_inactive, g.IO.DeltaTime * 6.f);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        RenderTextColor(font::lexend_bold, pos + ImVec2(12, 5), pos + ImVec2(GetContentRegionMax().x, 65*1.1f), GetColorU32(color_picker::text_active), label, ImVec2(0.0, 0.4 ));
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
        ...
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32, hue_color32, col_white, 5.f);
        draw_list->AddRectFilledMultiColorRounded(picker_pos - ImVec2(1, 1), picker_pos + ImVec2(sv_picker_size + 1, sv_picker_size + 1), 0, 0, col_black, col_black, 5.f);
        ...
        for (int i = 0; i < 6; ++i) draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6) - (i == 5 ? 1 : 0)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6) + (i == 0 ? 1 : 0)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1], 10.f, i == 0 ? ImDrawFlags_RoundCornersTop : i == 5 ? ImDrawFlags_RoundCornersBottom : ImDrawFlags_RoundCornersNone);
        ...
        draw_list->AddCircleFilled(ImVec2(bar0_pos_x + 7.5f, state.hue_bar), 4.5f, GetColorU32(color_picker::picker_circle), 100.f);

        float sv_cursor_rad = value_changed_sv ? 10.0f : 6.0f;
        int sv_cursor_segments = draw_list->_CalcCircleAutoSegmentCount(sv_cursor_rad);

        state.circle_move = ImLerp(state.circle_move, sv_cursor_pos, g.IO.DeltaTime * 15.f);
        state.circle = ImLerp(state.circle, value_changed_sv ? 8.0f : 6.0f, g.IO.DeltaTime * 30.f);
        ...
        if (alpha_bar)
        {
            ...
            draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min, bar1_bb.Max, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, 100.f);
            ...
            draw_list->AddCircleFilled(ImVec2(bar1_pos_x + 7.5f, state.alpha_bar), 4.5f, GetColorU32(color_picker::picker_circle), 100.f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect rect(pos + ImVec2(1, 1), pos + ImVec2(width , 60 * 1.1f));

    const ImRect clickable(rect.Min + ImVec2(width - 40 * 1.1f, 0 * 1.1f), rect.Max - ImVec2(10 * 1.1f, 35 * 1.1f));
 ...
 for (int i = shadow_steps; i > 0; i--)
 {
    ...
    // ИЗМЕНЕНИЕ: Фон кнопки цвета на 18,18,18
    GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, ImColor(18, 18, 18), 5);  // Было: ImColor(c::elements::background)

    // Круг с выбранным цветом остается без изменений
    GetWindowDrawList()->AddCircleFilled(clickable.Max + ImVec2(-17, 5.5f), 13.f * 1.1f, GetColorU32(col_rgb));
